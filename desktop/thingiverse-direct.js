...
function validateSubmit(input) { const issues = []; if (!String(input.name || '').trim()) issues.push('name is required'); if (!String(input.summary || '').trim()) issues.push('summary is required'); if (!input.categoryId) issues.push('category is required'); if (!LICENSES.has(input.license)) issues.push('license is required'); if (!(input.files || []).some((file) => file.role === 'model')) issues.push('at least one model file is required'); if (input.publish && !input.termsAccepted) issues.push('current terms must be accepted to publish'); if (input.remix && !String(input.sourceThingId || '').trim()) issues.push('remixes require a source Thing ID'); return issues; }
