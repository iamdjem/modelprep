...
import { fileSlicer, slicerLabel } from './threemf.js';
import { withoutExcluded } from './platform-files.js';
...
export function makerWorldPublishIssues(project, opts = {}, runtime = {}) {
  ...
  const files = withoutExcluded(makerWorldFilesForMode(project?.files || [], productMode), opts);
  ...
  for (const image of images) {
    if ((image.size || 0) > 30 * MB) errors.push(`${image.name || image.alt || 'Image'} exceeds MakerWorld's 30MB per-image limit.`);
  }
  ...
  const includedFiles = withoutExcluded(project?.files || [], opts);
  const primaryFile = includedFiles.find((file) => file.id === opts.primaryProfileFileId)
    || includedFiles.find((file) => fileExt(file.name) === '3mf' && fileSlicer(file) === 'bambu')
    || includedFiles.find((file) => fileExt(file.name) === '3mf');
  ...
  if (primaryFile && fileExt(primaryFile.name) === '3mf') {
    // MakerWorld rejects print profiles from other slicers after upload
    // ("The 3mf was not generated by Bambu Studio"), so fail closed here.
    const detected = fileSlicer(primaryFile);
    if (detected !== 'unknown' && detected !== 'bambu') {
